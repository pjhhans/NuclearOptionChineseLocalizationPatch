...
- **自带诊断窗口。** `F11` 呼出设置与诊断窗口，含「最近命中」与「最近漏译」。
...
> **默认只开翻译。** 兜底扫描、调试日志、漏译记录三项默认全关，也不写任何诊断文件
...
| 调节 | 兜底扫描开关与间隔、是否记录最近命中、是否累积漏译、是否输出调试日志（**除「记录最近命中」外默认全关**） |
...
**安装后的默认状态：只开翻译。** 兜底扫描、调试日志、漏译记录**全部默认关闭** ——
...
关掉它**不影响** `F11` 窗口里的「最近漏译」——那是内存里的 12 条环形缓冲，不落盘。
